...
È disponibile un'interfaccia per i comandi di linea per convertire gli originali checkpoint di Bert/GPT/GPT-2/Transformer-XL/XLNet/XLM 
...
Questo CLI prende come input un checkpoint di Tensorflow (tre files che iniziano con `bert_model.ckpt`) e i relativi 
file di configurazione (`bert_config.json`) e crea un modello Pytorch per questa configurazione, carica i pesi dal
checkpoint di Tensorflow nel modello di Pytorch e salva il modello che ne risulta in un file di salvataggio Pytorch che 
...
Puoi scaricare i modelli pre-allenati di Google per la conversione [qua](https://github.com/google-research/albert#pre-trained-models).
...
sia salvato nello stesso formato dei modelli pre-allenati OpenAI (vedi [qua](https://github.com/openai/finetune-transformer-lm)):
```bash
...
Ecco un esempio del processo di conversione di un modello OpenAI GPT-2 pre-allenato (vedi [qua](https://github.com/openai/gpt-2)):
...
(vedi [qua](https://github.com/kimiyoung/transformer-xl/tree/master/tf#obtain-and-evaluate-pretrained-sota-models)):
